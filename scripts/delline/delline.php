...
#print_r($s);		
	...
	if ($result->errors == NULL) {
    			$output= array('price' => (string)round($result->price), 'time' => $result->time->nominative, 'condition' =>$type[2].' (Наземный способ)');
    			echo json_encode($output). PHP_EOL;
    			if( $result->air!=NULL)echo json_encode(array('price' => (string)round($result->derival->price + $result->air->price), 'time' => $result->time->nominative, 'condition' =>$type[2].' (Авиаперевозка)')).PHP_EOL;
    	}
	}
